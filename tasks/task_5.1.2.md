...
docker exec -u root pfm-community-member-portal [command]        # Execute in member portal
docker exec -u root pfm-community-admin-dashboard [command]      # Execute in admin portal
docker restart pfm-community-member-portal                       # Restart container for updates
```

### File Creation and Distribution:
```bash
docker exec -u root [container] bash -c 'cat > [file_path] << "EOF"'  # Create files with content
docker cp [source] [destination]                                      # Copy files between containers
docker exec -u root [container] mkdir -p [directory]                  # Create directory structures
```

### Testing and Validation:
```bash
npm test -- --testNamePattern="Portal Integration Tests" --verbose  # Run integration tests
curl -s -w "HTTP Status: %{http_code}" [url]                       # HTTP endpoint testing
docker logs [container] --tail [number]                            # Check container logs
```
